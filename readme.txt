...
Calls remove_two_marbles() or remove_one_marble() depending on the player's choice, then triggers computer_turn()

6. Computer AI (Minimax)
computer_turn(pile_contents, pile_name)
Uses the Minimax algorithm with alpha-beta pruning to determine the best move for the computer by constructing a GameState
...
Displays the choices for the players

10. Main Functionn
main()
Accepts command-line arguments for number of red and blue marbles *INCOMPLETE*

How the is run: *INCOMPLETE* Need to add [version] [first-player] [d]
To run the code open up the terminal and insert this for example:
python red_blue_nim.py 10 10
The first number is the number of red chips and the second number is the number of blue chips.
If the total number of marbles is less than 16 the code will not run
